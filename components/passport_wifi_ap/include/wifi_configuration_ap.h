...
public:
    ...
    /**
     * Set callback for when WiFi credentials are submitted via the config web page.
     * Called from the HTTP handler after Save() succeeds. The connection itself is
     * left to the caller (main/wifi_sta.c), so the AP can be torn down promptly.
     */
    void OnCredentials(std::function<void(const std::string &ssid, const std::string &password)> callback);
...
private:
    ...
    std::function<void(const std::string &ssid, const std::string &password)> on_credentials_;
