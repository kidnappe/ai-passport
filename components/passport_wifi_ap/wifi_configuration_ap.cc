...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // 获取当前对象
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            // o-platform 移植: 不在 APSTA 模式下试连(原版 ConnectToWifi 会阻塞 10s+
            // 且吃内存), 只保存凭证并回调上层, 由 main/wifi_sta.c 在 AP 关闭后连接。
            this_->Save(ssid_str, password_str);
            if (this_->on_credentials_) {
                this_->on_credentials_(ssid_str, password_str);
            }
            cJSON_Delete(json);
            // 设置成功响应
            httpd_resp_set_type(req, "application/json");
...
void WifiConfigurationAp::OnCredentials(std::function<void(const std::string &ssid, const std::string &password)> callback)
{
    on_credentials_ = callback;
}
