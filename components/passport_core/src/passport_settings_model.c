...
uint16_t passport_settings_model_prev(passport_setting_id_t id, uint16_t current)
{
    ...
    switch (id) {
    case PASSPORT_SETTING_BRIGHTNESS:
        return current <= 10U ? 100U : current - 10U;
    case PASSPORT_SETTING_VOLUME:
        return current == 0U ? 100U : current - 10U;
    case PASSPORT_SETTING_SCREEN_TIMEOUT:
        if (current == 30U) return 0U;
        if (current == 0U) return 300U;
        if (current == 300U) return 180U;
        if (current == 180U) return 60U;
        return 30U;
    case PASSPORT_SETTING_KEY_SOUND:
        return current ? 0U : 1U;
    default:
