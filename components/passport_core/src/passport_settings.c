...
static void (*s_screen_cb)(bool off);
...
{
    /* 按键音、音量预览与语音录入共用同一份 16k/16/1 懒初始化守卫(幂等),
     * 保证 bsp_audio 只被初始化一次、全程不切换采样格式。 */
    return ensure_audio_ready();
}
...
static void check_screen_timeout(void)
{
    ...
    if (s_screen_cb) s_screen_cb(true);
...
esp_err_t passport_settings_cycle(passport_setting_id_t id, int delta)
{
    ...
    if (delta < 0) {
        return passport_settings_set(id, passport_settings_model_prev(id, current));
    }
