...
uint16_t passport_settings_model_prev(passport_setting_id_t id, uint16_t current);
...
esp_err_t passport_settings_cycle(passport_setting_id_t id, int delta);
...
/* 注册屏幕状态回调：息屏/亮屏时通知。传入 NULL 取消注册。 */
void passport_settings_set_screen_callback(void (*cb)(bool off));

/** Queue one non-blocking key click. The worker lazily initializes audio. */
void passport_settings_key_feedback(void);

/**
 * Ensure the audio codec is initialized at 16 kHz/16-bit/mono (the shared
 * format for key sounds, tone feedback and voice capture). Idempotent; safe
 * to call from any task. Returns false when codec bring-up failed.
 */
bool passport_settings_ensure_audio_ready(void);
