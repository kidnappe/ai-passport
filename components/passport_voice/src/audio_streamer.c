// audio_streamer.c —— 流式音频管线实现(移植自 folo-ai-passport-voice)。
...
// 音频帧发送函数(通道无关,见 audio_streamer.h)。注册点:passport_voice 初始化。
// 默认 NULL:发送一律按失败处理(丢帧计数),管线仍健康。
static audio_send_fn_t s_send_fn = NULL;
// 管线异常回调(采集硬件错误 → 上层收束会话)。worker 任务上下文,只投递。
static audio_streamer_evt_cb_t s_evt_cb;
...
static void streamer_notify_error(void) {
    if (s_evt_cb) s_evt_cb(AUDIO_STREAMER_EVT_ERROR, s_evt_user);
}
        ...
        while (s_active) {
            ...
            if (e != ESP_OK) {
                ESP_LOGE(TAG, "bsp_audio_read 失败 (%s),停流",
                         esp_err_to_name(e));
                streamer_notify_error();   // 上层收束会话回 READY
                s_active = false;
                break;
            }
            ...
            *(uint32_t *)s_chunk = my_token;
            if (xRingbufferSend(s_ring, s_chunk, CHUNK_ITEM_BYTES, 0) == pdTRUE) {
                ...
                // 破坏 100ms 采集节奏;麦克风继续跑,I2S 驱动自动落样)。
                // 拥塞恢复由 ble_worker 发送侧解除;drop 计数随 status 帧对账。
                drop_inc();
            }
        }
        s_worker_busy = false;
