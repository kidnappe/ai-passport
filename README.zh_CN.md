...
| **Wi‑Fi 配网** | 先后两版：**① 早期 BLE 配网** [`killhello/ai-pass-port-wifi`](https://github.com/killhello/ai-pass-port-wifi) → **② 现行热点配网（小智）** [`78/esp-wifi-connect`](https://github.com/78/esp-wifi-connect)（softAP + captive portal） | MIT | 早期 `main/ble_prov.c`（BLE 配网，现 BLE 仅承载语音通道）→ 现行 `components/passport_wifi_ap/` |
...
**电子工牌：** 主页显示工牌字段与头像；进「传输」页（设备自开免密热点，手机连上自动弹出编辑页）改字段/头像，见下文「编辑电子工牌」。
...
主页工牌有 **昵称 + 三行自定义字段**（默认标签"学院/专业/学号"，可改成任意），每行可设**内容、字号(14/24)、颜色、粗体**；**内容为空的行整行不显示**。另可上传一张**静态头像照片**。

改法（像配网一样，设备自开热点）：
1. 设备进「传输」页 → 按 OK → 设备开一个**免密热点 `Passport-Set-xxxx`**（屏幕显示热点名）。
2. 手机连接该热点 → 会**自动弹出编辑页**（captive portal）；若没自动弹，浏览器手动开 `http://192.168.4.1/`。
3. 改昵称/三行（自定义标签、内容、字号、颜色、粗体）→ 点「保存到设备」（**原生表单提交**，任何浏览器/弹页都能用）→ 出现**绿色 ✓「保存成功」**页 → **回设备主页即生效**。
4. **静态头像**需在**能跑 JS 的浏览器（如 Chrome）打开 `http://192.168.4.1/`** 用「转换并上传头像」（前端把图转 RGB565 再传）；上传后自动切静态。想回动态人物：编辑页选「动态人物」，或「设置 → 动态头像」重新打开。

字段存于 `/passport/{名}.txt`、`{名}_label.txt`（标签）、`{名}_sz/_color/_bold.txt`，头像 `/passport/avatar.raw`，静态/动态开关为 NVS `pass_net/dyn`。
> 中文字库只有 **14/24px 两档**、为 **GB2312 常用字子集**，字库外生僻字显示空白。
